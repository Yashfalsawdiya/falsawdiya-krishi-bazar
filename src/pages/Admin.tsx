  ...
  React.useEffect(() => {
    if (appContent) {
      ...
      setContentForm({
        ...
        festivalOffer: {
          show: false,
          title: 'दिवाली महा धमाका ऑफर',
          subtitle: 'सभी कृषि उत्पादों पर 20% तक की भारी छूट!',
          image: '',
          theme: 'diwali',
          link: ''
        }
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-[#4A3728] flex items-center gap-2">
                ...
                स्पेशल ऑफर्स (Special Offers Section)
              </h3>
              <div className="flex items-center gap-4">
                <button 
                  type="button"
                  onClick={() => {
                    if (!contentForm) return;
                    setContentForm({...contentForm, offers: {...(contentForm.offers || {show: false, title: '', items: []}), show: !contentForm.offers?.show}});
                  }}
                  className={cn(
                    "w-12 h-6 rounded-full relative transition-colors duration-200",
                    contentForm?.offers?.show ? "bg-[#2D5A27]" : "bg-gray-300"
                  )}
                >
                  <div className={cn(
                    "absolute top-1 w-4 h-4 bg-white rounded-full transition-all duration-200",
                    contentForm?.offers?.show ? "left-7" : "left-1"
                  )} />
                </button>
                <button 
                  type="button"
                ...
                >
                  <Plus className="w-3 h-3" /> ऑफर कार्ड जोड़ें
                </button>
              </div>
            </div>

            {contentForm?.offers?.show && (
              <div className="bg-white p-5 rounded-3xl shadow-sm border border-gray-100 space-y-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">सेक्शन टाइटल (Section Title)</label>
                  <input 
                    type="text" 
                    value={contentForm?.offers?.title || ''}
                    onChange={e => {
                      if (!contentForm) return;
                      setContentForm({...contentForm, offers: {...(contentForm.offers || {show: true, title: '', items: []}), title: e.target.value}});
                    }}
                    className="w-full bg-gray-50 border-2 border-transparent focus:border-[#2D5A27] focus:bg-white rounded-2xl p-4 outline-none transition-all font-medium"
                ...
                <div className="grid grid-cols-1 gap-4">
                  {contentForm?.offers?.items.map((item, idx) => (
                    <div key={item.id} className="p-4 bg-gray-50 rounded-2xl border border-gray-100 relative group">
                      <button 
                        type="button"
                        onClick={() => {
                          if (!contentForm?.offers) return;
                          const newItems = contentForm.offers.items.filter(i => i.id !== item.id);
                          setContentForm({...contentForm, offers: {...contentForm.offers, items: newItems}});
                        }}
                        className="absolute top-2 right-2 p-1.5 text-red-500 hover:bg-red-100 rounded-lg transition-colors"
                      >
                      ...
                      <div className="space-y-3">
                        <div className="space-y-1">
                          <label className="text-[9px] font-bold text-gray-400 uppercase tracking-widest ml-1">ऑफर टाइटल (Offer Title)</label>
                          <input 
                            type="text" 
                            value={item.title}
                            onChange={e => {
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-[#4A3728] flex items-center gap-2">
                ...
                फेस्टिवल ऑफर (Festival Offer Section)
              </h3>
              <button 
                type="button"
                onClick={() => {
                  ...
                  setContentForm({
                    ...contentForm, 
                    festivalOffer: {
                      ...(contentForm.festivalOffer || { show: false, title: '', subtitle: '', image: '', theme: 'general' }), 
                      show: !contentForm.festivalOffer?.show 
                    }
                  });
                }}
                className={cn(
                  "w-12 h-6 rounded-full relative transition-colors duration-200",
                  contentForm?.festivalOffer?.show ? "bg-[#2D5A27]" : "bg-gray-300"
                )}
              >
                <div className={cn(
                  "absolute top-1 w-4 h-4 bg-white rounded-full transition-all duration-200",
                  contentForm?.festivalOffer?.show ? "left-7" : "left-1"
                )} />
              </button>
            </div>

            {contentForm?.festivalOffer?.show && (
              <div className="bg-white p-5 rounded-3xl shadow-sm border border-gray-100 space-y-4 animate-in fade-in slide-in-from-top-2">
                <div className="grid grid-cols-1 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">त्योहार का नाम/टाइटल (Festival Name/Title)</label>
                    <input 
                      type="text" 
                      value={contentForm?.festivalOffer?.title || ''}
                      onChange={e => {
                        if (!contentForm) return;
                        setContentForm({...contentForm, festivalOffer: {...(contentForm.festivalOffer || {show: true, title: '', subtitle: '', image: '', theme: 'general'}), title: e.target.value}});
                      }}
                      className="w-full bg-gray-50 border-2 border-transparent focus:border-[#2D5A27] focus:bg-white rounded-2xl p-4 outline-none transition-all font-medium"
                      placeholder="जैसे: दिवाली महा धमाका ऑफर"
                    />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">ऑफर की जानकारी (Offer Information)</label>
                    <input 
                      type="text" 
                      value={contentForm?.festivalOffer?.subtitle || ''}
                      onChange={e => {
                        if (!contentForm) return;
                        setContentForm({...contentForm, festivalOffer: {...(contentForm.festivalOffer || {show: true, title: '', subtitle: '', image: '', theme: 'general'}), subtitle: e.target.value}});
                      }}
                      className="w-full bg-gray-50 border-2 border-transparent focus:border-[#2D5A27] focus:bg-white rounded-2xl p-4 outline-none transition-all font-medium"
                      placeholder="जैसे: सभी बीजों पर 15% की छूट"
                    />
                  </div>
                  
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">फेस्टिवल थीम (Festival Theme)</label>
                    <div className="grid grid-cols-3 gap-2">
                      {(['diwali', 'holi', 'rakhi', 'navratri', 'monsoon', 'general'] as const).map((theme) => (
                        <button
                          key={theme}
                          type="button"
                          onClick={() => {
                            if (!contentForm) return;
                            setContentForm({...contentForm, festivalOffer: {...(contentForm.festivalOffer || {show: true, title: '', subtitle: '', image: '', theme: 'general'}), theme}});
                          }}
                          className={cn(
                            "py-2 rounded-xl text-[10px] font-bold uppercase transition-all border-2",
                            contentForm?.festivalOffer?.theme === theme 
                        ...
                        >
                          {theme === 'diwali' ? 'दिवाली' : 
                           theme === 'holi' ? 'होली' : 
                           theme === 'rakhi' ? 'रक्षाबंधन' : 
                           theme === 'navratri' ? 'नवरात्रि' :
                           theme === 'monsoon' ? 'मानसून' : 'सामान्य'}
                        </button>
                      ))}
                    </div>
                  </div>
