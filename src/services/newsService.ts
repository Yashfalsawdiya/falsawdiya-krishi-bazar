...
// Dynamic fallback news relative to today
const generateFallbacks = (): AgriNewsItem[] => {
  return [
    ...
    {
      title: "मंडी भाव अपडेट: मालवा क्षेत्र की मंडियों में सोयाबीन और लहसुन में तेजी",
      summary: "मध्य प्रदेश के मालवा-निमाड़ अंचल की प्रमुख अनाज मंडियों (मंदसौर, नीमच, शामगढ़) में सोयाबीन और लहसुन की आवक में अच्छी तेजी देखी जा रही है। मंडियों में प्रतिदिन हजारों क्विंटल उपज की नीलामी हो रही है और मांग अधिक होने से भावों में मजबूती बनी हुई है। किसानों को मंडी में आने से पहले अपनी फसल को अच्छी तरह साफ करने और सुखाने की सलाह दी गई है।",
      date: getOffsetDateStr(0),
      source: "मंडी रिपोर्ट",
    ...
    {
      title: "धान और दलहन की उन्नत किस्मों पर किसानों को मिलेगी ट्रेनिंग",
      summary: "स्थानीय कृषि विज्ञान केंद्रों (KVK) द्वारा किसानों के लिए रोग-प्रतिरोधी तथा कम पानी में पकने वाली उन्नत किस्मों पर विशेष प्रशिक्षण सत्रों का आयोजन किया जा रहा है। इन व्यावहारिक शिविरों में किसानों को वैज्ञानिक नर्सरी प्रबंधन, उचित पौध दूरी, एकीकृत पोषक तत्व प्रबंधन और जल संरक्षण की आधुनिक तकनीकों के बारे में प्रशिक्षण दिया जाएगा।",
      date: getOffsetDateStr(2),
      source: "Kisan News",
    ...
    {
      title: "मध्य प्रदेश में फसलों की एमएसपी और भावांतर भुगतान की प्रक्रिया तेज",
      summary: "सोयाबीन एवं दलहन फसलों के उपार्जन को लेकर मध्य प्रदेश सरकार ने मंडियों में किसानों के पंजीकरण एवं भावांतर भुगतान योजना के तहत त्वरित भुगतान सुनिश्चित करने के निर्देश दिए हैं। किसान संगठनों ने इस कदम का स्वागत किया है।",
...
export const fetchAgriNews = async (userApiKey?: string, forceRefresh: boolean = false): Promise<AgriNewsResponse> => {
  ...
  // If no cache or all items in cache are older than 7 days, initialize cache with fresh fallbacks
  const hasRecent = currentCache.some(item => {
    try {
  ...
  if (currentCache.length === 0 || !hasRecent) {
  ...
  try {
    const res = await fetch('/api/ai/agri-news', {
      method: 'POST',
      headers: {
    ...
    if (res.ok) {
      ...
      if (data.success && Array.isArray(data.items) && data.items.length > 0) {
        const formattedFetched = data.items.map((item: any) => ({
          title: item.title || 'कृषि समाचार',
          summary: item.summary || '',
          date: item.date || todayStr,
          source: item.source || 'कृषि समाचार',
          url: item.url || 'https://krishijagran.com/',
    ...
    console.warn("Server AI agri-news fetch failed, trying client fallback...", apiErr);
  }

  // 2. Client SDK fallback if API Key is available
  try {
    ...
    let newlyFetched: AgriNewsItem[] = [];
    try {
      const rawText = response.text?.replace(/```json/g, '')?.replace(/```/g, '')?.trim() || '[]';
      newlyFetched = JSON.parse(rawText);
    } catch (e) {
      console.warn("Failed to parse newly fetched news JSON, using fallbacks:", e);
      newlyFetched = generateFallbacks();
    }
